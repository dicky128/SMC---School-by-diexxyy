...
══════════════════════════════════════════════════════════ */
(function initNavbar() {
  // HTML navbar memakai id="navbar" (lihat includes/header.php)
  const nav = document.getElementById('navbar');
  if (!nav) return;

  window.addEventListener('scroll', () => {
    // toggle class untuk effect CSS
    nav.classList.toggle('scrolled', window.scrollY > 40);

    // Frosted glass on scroll (jaga kompatibilitas dengan CSS/JS styling)
    if (window.scrollY > 40) {
      nav.style.background = nav.style.background || 'rgba(0,0,0,0.88)';
      nav.style.backdropFilter = 'blur(24px) saturate(2)';
      nav.style.webkitBackdropFilter = 'blur(24px) saturate(2)';
    } else {
