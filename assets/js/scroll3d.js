...
/**
 * assets/js/scroll3d.js — SD Muhammadiyah 01 Gentasari
 * Premium scroll-driven depth effects
 * Versi Final — Ringan, performa optimal
 */
(function () {
    'use strict';

    // Guard: only run on non-touch devices
    if (!window.matchMedia('(pointer: fine)').matches) return;

    /* ── Depth parallax for depth-section elements ───────────── */
    ...
    if (depthEls.length) {
        const handleDepth = () => {
            ...
            depthEls.forEach(el => {
                ...
                const offset = (window.innerHeight / 2 - center) * depth;
    ...
    /* ── Section entrance with IntersectionObserver ──────────── */
    const sections = document.querySelectorAll('.depth-section');
    ...
    sections.forEach(sec => {
        ...
        if (!sec.classList.contains('no-entrance')) {
            sectionObserver.observe(sec);
        }
    });

    /* ── Floating elements subtle drift ─────────────────────── */
    const floaters = document.querySelectorAll('.animate-float-y, .animate-float-diagonal');
    // These already use CSS keyframe animations — no JS needed.
    // This space reserved for GSAP upgrades if added later.
